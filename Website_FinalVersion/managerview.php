...
<div class="Butbar">		
		<form method = "post">
			...
			<br>
			<li><button class="large button" type = "submit" name = "branchview">Branch Staff</button></li>
			<br>
...
<?php 
		...
		{
			include "db.php";
			$querystring = "SELECT * FROM staffview WHERE BranchNumber = '" . $_SESSION['branchno'] . "'";
			$sql = $pdo->prepare($querystring);
			$sql->execute();
			echo "<table>
			...
			while ($row = $sql->fetch(PDO::FETCH_ASSOC))
			{
				...
				echo "<tr><td>$staffno</td><td>$fname</td><td>$lname</td><td>$sex</td><td>$address</td><td>$email</td><td>$phone</td><td>$role</td><td>£$salary</td><td>$nin</td>";
				if ($role == "Sales") {echo "<td><form method = \"post\"><button type = \"submit\" value = \"$staffno\" name = \"deleterecord\">Delete</button></form></td>";}
			}
		echo "</table>";
		}
		...
		if ($availability == 1)
		{
			$querystring = "SELECT * FROM carstockview WHERE make LIKE ? and type LIKE ? and transmission LIKE ? and availability = 1";
			$params = array("%$makepref%", "%$typepref%", "%$transpref%");
		}
		else if ($availability == 0)
		{
			$querystring = "SELECT * FROM carstockview WHERE make LIKE ? and type LIKE ? and transmission LIKE ? and availability = 0";
			$params = array("%$makepref%", "%$typepref%", "%$transpref%");
		}
		else
		{
			$querystring = "SELECT * FROM carstockview WHERE make LIKE ? and type LIKE ? and transmission LIKE ?";
			$params = array("%$makepref%", "%$typepref%", "%$transpref%");
		}
		$sql = $pdo->prepare($querystring);
		$sql->execute($params);
		...
		echo "<div class='sidesearchmanager'> ";
		...
		function leaseviewSwitch()
		{
			...
			if ($date != "")
			{
				...
				$querystring = "SELECT * FROM leaseview WHERE ? BETWEEN start_date AND end_date and BranchNumber = ?";
				$params = array("$date", "$branchno");
			}
			else
			{
				$querystring = "SELECT * FROM leaseview WHERE BranchNumber = ?";
				$params = array("$branchno");
			}
		...
		function insertviewSwitch()
		{
			...
			if(isset($_POST['setedit']))
			{
				switch($addview)
				{
					...
					$sql->execute(array(':branchno' => $_SESSION['branchno']));
					$succeed = False;
					while ($row = $sql->fetch(PDO::FETCH_ASSOC))
					{
						$succeed = True;
					...
					if ($succeed)
					{
						...
						$updated = $sql->execute(array(':managerid' => $_POST['managerid'], ':branchno' => $_SESSION['branchno']));
					...
					{					
						...
						if($updated)
						{
							...
							$updated2 = $sql->execute(array(':branchno' => $_SESSION['branchno'], ':addressid' => $_SESSION['lastestaddressid'], ':managerid' => $_POST['managerid']));
					...
					if ($succeed)
					{
						...
						$updated = $sql->execute(array($_SESSION['branchno'], $_POST['insuranceno'], $_POST['supplierno'], $_POST['make'], $_POST['model'], $_POST['type'], $_POST['transmission'], $_POST['rent'], $availability, $_POST['lastcheckup'], $_POST['colour'], $_POST['regplate'], $_POST['vin']));
						if ($updated)
						{
							$_SESSION['feedbackmessage'] = "Record updated/added";
						}
					...
					else
					{
						...
						$updated = $sql->execute(array($_POST['vin'], $_SESSION['branchno'], $_POST['insuranceno'], $_POST['supplierno'], $_POST['make'], $_POST['model'], $_POST['type'], $_POST['transmission'], $_POST['rent'], $availability, $_POST['lastcheckup'], $_POST['colour'], $_POST['regplate']));
						if ($updated)
						{
							$_SESSION['feedbackmessage'] = "Record updated/added";
						}
					...
					if ($succeed)
					{
						...
						$updated = $sql->execute(array(':branchno' => $_SESSION['branchno'], ':phoneno' => $_POST['phoneno'], ':companyname' => $_POST['companyname'], ':companyno' => $_POST['companyno']));
					...
					else
					{
						...
						if($updated)
						{
							...
							$updated = $sql->execute(array(':branchno' => $_SESSION['branchno'], ':companyno' => $_POST['companyno'], ':addressid' => $_SESSION['lastestaddressid'], ':phoneno' => $_POST['phoneno'], ':companyname' => $_POST['companyname']));
					...
					else
					{
						...
						if ($succeed)
						{
							...
							$updated = $sql->execute(array(':branchno' => $_SESSION['branchno'], ':fname' => $_POST['fname'], ':lname' => $_POST['lname'], ':sex' => $_POST['sex'], ':email' => $_POST['email'], ':phoneno' => $_POST['phoneno'], ':role' => $_POST['role'], ':salary' => $_POST['salary'], ':nin' => $_POST['nin'], ':staffno' => $_POST['staffno']));
						...
						else
						{
							...
							if($updated)
							{
								...
								$updated2 = $sql->execute(array(':staffno' => $_POST['staffno'], ':branchno' => $_SESSION['branchno'], ':addressid' => $_SESSION['lastestaddressid'], ':fname' => $_POST['fname'], ':lname' => $_POST['lname'], ':sex' => $_POST['sex'], ':email' => $_POST['email'], ':phoneno' => $_POST['phoneno'], ':role' => $_POST['role'], ':salary' => $_POST['salary'], ':nin' => $_POST['nin']));
					...
					else
					{
						...
						if($updated)
						{
							...
							$updated2 = $sql->execute(array(':branchno' => $_SESSION['branchno'], ':companyno' => $_POST['companyno'], ':addressid' => $_SESSION['lastestaddressid'], ':phoneno' => $_POST['phoneno'], ':companyname' => $_POST['companyname']));
			...
			switch($addview)
			{
				...
				case "adverts":
						echo "	<table>
							...
							<form method = \"post\">
								...
								<td><input type=\"date\" name=\"datepublished\"></td>
				...
				case "branch":
						echo "	<table>
							<tr><td>Property Number</td><td>Street</td><td>City</td><td>Postcode</td><td>Manager Number</td>
							<form method = \"post\">
								<tr>
				...
				case "car":
						echo "	<table>
							<tr><td>Vehicle Identification Number</td><td>Insurance Number</td><td>Supplier Number</td><td>Make</td><td>Model</td><td>Type</td><td>Transmission</td><td>Daily Rent</td><td>Available</td>
							<form method = \"post\">
								<tr>
								<td><input type=\"text\" name=\"vin\"></td>
								<td><input type=\"text\" name=\"insuranceno\"></td>
								<td><input type=\"text\" name=\"supplierno\"></td>
								...
								<tr>
								<td>Last Checkup</td><td>Colour</td><td>Registration Plate</td>
								<tr>
				...
				case "repair":
						echo "	<table>
							<tr><td>Company Number</td><td>Property Number</td><td>Street</td><td>City</td><td>Postcode</td><td>Phone Number</td><td>Company Name</td>
							<form method = \"post\">
								<tr>
				...
				case "staff":
						echo "	<table>
							<tr><td>Staff Number</td><td>Property Number</td><td>Street</td><td>City</td><td>Postcode</td><td>First Name</td><td>Last Name</td><td>Sex</td><td>Email</td>
							<form method = \"post\">
								<tr>
								...
								<tr>
								<td>Phone Number</td><td>Role</td><td>Salary</td><td>National Insurance Number</td>
								<tr>
								<td><input type=\"text\" name=\"phoneno\"></td>
		...
		function updateaddress()
		{
			...
			$sql = $pdo->prepare("UPDATE address SET property_number = :propertyno, street = :street, city = :city, postcode = :postcode WHERE address_id = :addressid");
			$updated = $sql->execute(array(':propertyno' => $_POST['propertyno'], ':street' => $_POST['street'], ':city' => $_POST['city'], ':postcode' => $_POST['postcode'], ':postcode' => $_POST['postcode'], ':addressid' => $addressid));
			return $updated;
		}
